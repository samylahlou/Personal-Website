...
The Jacobian is really the generalization of the derivative. The intuitive way of thinking about the Jacobian is to view it as the linear approximation of the function at a given point (up to translation). Hence, it should be no surprise that it satisfies most of the properties of the usual derivative:
...
\begin{proposition} Let $f, g : U \subseteq \R^n \to \R^m$ be two functions differentiable at $p \in U$.
    \begin{enumerate}
        ...
        \item The function $\lambda f$ is differentiable at $p$, and $D_p(\lambda f) = \lambda D_pf$ for all $\lambda \in \R$.
        \item The function $f + g$ is differentiable at $p$, and $D_p(f + g) = D_pf + D_pg$.
        \item The function $fg$ is differentiable at $p$, and $D_p(fg) = g(p)D_pf + f(p)D_pg$.
        \item If $g(p) \neq 0$, then the function $f/g$ is differentiable at $p$, and
        $$D_p\left(\frac{f}{g}\right) = \frac{g(p)D_pf - f(p)D_pg}{g(p)^2}.$$
    \end{enumerate}
\end{proposition}

The formulas above should be compared to their one-variables analogue to see that they are really the same. Another important property that really deserves its own theorem is the Chain Rule.
...
\begin{theorem}[Inverse Function Theorem]
    Suppose $\tilde{U} \subseteq \R^m$ is open, $f : \tilde{U} \to \R^m$ is $C^k$, and $q \in \tilde{U}$ satisfies $D_qf : \R^m \to \R^m$ is invertible ($\iff \det D_q f \neq 0$), then there are open subsetes $U \subseteq \tilde{U}$ and $V \subseteq \R^m$ such that $q \in U$, $f(U) = V$, $f|_U : U \to V$ is one-to-one with a $C^k$-inverse $g : V \to U$ and $D_{f(q)}g = D_qf^{-1}$. \td
\end{theorem}
...
\begin{definition}
    Let $U \subseteq \R^m$ be open, $F : U \to \R^n$ be a $C^1$ function. $p \in U$ is a \textit{critival point} of $F$ if the jacobian at $p$ is not onto (iff the jacobian has rank $< n$). $r \in \R^n$ is a \textit{regular value} if it is not the image of a critical point : $\R^n \setminus F(\{\text{critical points}\})$.
\end{definition}
